...
import logo from '../assets/images/Taqtile_Vertical_Black-removebg-preview.png';
export default function Home() {
    ...
    const [selectedManualPath, setSelectedManualPath] = useState('../assets/manuals/WMF1000manual.pdf');
    const [disableNewRequest, setDisableNewRequest] = useState(false);
    const [splittedDocs, setSplittedDocs] = useState();
    const onAsk = async () => {
        const response = await fetch('api/pdf', {
            method: 'POST',
            body: JSON.stringify({ filepath: '../assets/manuals/WMF1000manual.pdf' }),
            headers: {
                "Content-Type": "application/json"
            }
        });
        setChatMessages(prev => [...prev, JSON.stringify(response)]);
    };
    return (<div style={{
            display: 'flex',
          ...
          <div style={{ position: 'absolute', top: '32px', right: '32px' }}>
              ...
              <div>{JSON.stringify(splittedDocs)}</div>
              <select style={{
            borderRadius: '8px',
            border: 'none',
            height: '32px',
            ...
            ? (<div style={{ height: '75%', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', rowGap: '32px' }}>
                    ...
                    return <div style={{ display: 'flex', maxWidth: '75%', borderRadius: '12px', backgroundColor: index % 2 ? 'rgba(238,127,49,0.75)' : 'rgba(0,0,0,0.25)', alignSelf: index % 2 ? 'flex-end' : 'flex-start', padding: '8px 12px 8px 12px', color: '#FFFFFF' }}>
                      ...
                      <div style={{ display: 'flex', columnGap: '12px', width: '50%' }}>
                          <input onSubmit={e => e.preventDefault()} placeholder='Enter your prompt here' style={{
                    fontSize: '16px',
                    ...
                    width: '100%'
                          ...
                          <button onClick={() => onAsk()} disabled={disableNewRequest} style={{
                    color: '#FFFFFF',
